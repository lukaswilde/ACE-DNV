...
def extract_gaze_features_single(x, y, targets):
    ...
    for i in range(ini, len(x) - 1):
        ...
        # saving direction % window
        tr_tensor[i][1] = np.atan2(diff_y, diff_x)
    if targets is not None:
        tgt_tensor = targets
    else:
        tgt_tensor = None
    ...
    for i in range(ini, len(x)):
        for j in range(len(windows)):
            ...
            if start_pos == end_pos:
                continue
            diff_x = x[end_pos] - x[start_pos]
            ampl = np.sqrt(diff_x**2)
            time = ((end_pos - start_pos) * latency) / 1000
            # saving speed
            tr_tensor[i - ini][j] = ampl / time
            # saving direction % window
            tr_tensor[i - ini][j + len(windows)] = np.atan(diff_x)
    return tr_tensor
...
def preprocessor(gaze, patchSim, headRot, bodyLoc, gaze_t, frame_t, labels, lblMatch):
    ...
    if labels is not None:
        ...
        labels[labels == 1] = 0
        lblMatch[lblMatch == 1] = 0
        # Gaze Pursuit = 1 (originally 2),
        labels[labels == 2] = 1
        lblMatch[lblMatch == 2] = 1
        # Saccade = 2 (originally 3),
        labels[labels == 3] = 2
        lblMatch[lblMatch == 3] = 2
        # Gaze Following = 3 (originally 5)
        labels[labels == 5] = 3
        lblMatch[lblMatch == 5] = 3
...
def data_balancer_nonRandom(x, y):
    ...
    for c in range(len(ulbls)):
        ...
        remain = min_count
        while True:
            record_extract_factor = np.round(
                remain / (len(np.where(delection_counts > 0)[0]) + 0.0000001),
            )
    ...
    # remove samples
    for c in range(len(ulbls)):  # each class
    ...
    for r in range(len(x)):
        ...
        for c in range(len(ulbls)):
            ...
            for e in range(events.shape[0]):
                ...
                lb = events[e, 2]
                leng = event_lens[e]
                if goal_diffs_ratio[lb] == 0:
                    continue
                if leng < 3:
                    continue
                if int(np.round(leng * goal_diffs_ratio[lb]) - 1) < 0:
                    continue
                rm_is = random.sample(
                    range(start, end),
                    int(np.round(leng * goal_diffs_ratio[lb]) - 1),
                )
...
def divider_twoChunkSplit(X, Y):
    ...
    for i in range(X.size):
        ...
        rec = np.delete(
            rec,
            np.arange(randStart, (randStart + recLen)),
            axis=0,
